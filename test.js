...
//how to filter out the categories of products from the api data
const choices = [...new Set([...products.map((p) => p.category)])]; // to to each category
choices.map((c) => ({
  value: c,
  label: c.split("-").join(" "),
  checked: false,
}));
// it will give thed 6 category based on the above data
// filter brach under in products
//spread operator used in arr form
//quoka js for live editor veiwing
const brand = new Set([...products.map((b) => b.brands)]);
